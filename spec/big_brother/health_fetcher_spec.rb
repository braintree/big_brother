...
HTTP
      BigBrother::HealthFetcher.interpol_status(Factory.node(:address => "127.0.0.1", :port => 8081, :path => "/test/status", :interpol => true)).should == [
        {"aggregated_health" => 0,"count" => 1,"lb_ip_address" => "load1.stq","lb_url" => "http://load1.stq:80/lvs.json","health" => 0}
      ]
    end
...
HTTP
    ...
    it "returns 0 for an unknown DNS entry" do
      BigBrother::HealthFetcher.interpol_status(Factory.node(:address => "junk.local", :port => 8081, :path => "/test/status", :interpol => true)).should == []
    end
